...
public class ReconcileIdentityAttributes extends CliUtil {
	
	private static ReconcileManagerRemote recm;
	
	public ReconcileIdentityAttributes(ReconcileManagerRemote recm) {
		setRecm(recm);
	}
	...
	private static void setRecm(ReconcileManagerRemote recm) {
		ReconcileIdentityAttributes.recm = recm;
	}
	...
	public void start(String[] args) {
		...
		try {
			...
			System.out.println("Executing Reconcile Identity Attributes Process");
			...
			try {
				if (type.toLowerCase().equals("full")) {
					...
					getRecm().reconcileIdentityAttributes();
				}
				else {
				...
				System.out.println("Sucessfully requrested identity attributes reconciliation");
				System.out.println("Please follow internal logs for detailed information.");
			}
			catch (ReconcileProcessException rue) {
				System.err.println("Failed to reconcile identity attributes, exception is: " + rue.getMessage());
			}
		}
